...
from loss_fn.pose_loss import PoseLoss
from loss_fn.edge_loss import SobelLoss
...
class Trainer():
	def __init__(self, model, train_loader, val_loader, normalize_mean, normalize_std, args):
		...
		self.scheduler          = torch.optim.lr_scheduler.CosineAnnealingLR(self.optimizer, T_max=T_max, eta_min=args.init_lr*0.2)
		
		# warmup
		self.warmup_step = 2000 # 1000
		...
		self.use_pose_loss          = False
		self.use_sobel_loss         = False
		...
		self.pose_loss_weight          = 40.
		self.sobel_loss_weight         = 0.25
	...
	def __setup_metric__(self):
		...
		if self.use_pose_loss:
			self.pose_loss = self.init_pose_loss(yolo_path='./saved_checkpoint/yolov8s.pt', 
												model_path='./saved_checkpoint/vitpose-b-coco.pth', 
												model_sz='b',
												max_batch_size=args.batch_size*8)
			self.add_metric(self.train_metric, "pose_loss")
			self.add_metric(self.val_metric, "pose_loss")
		...
		if self.use_sobel_loss:
			self.sobel_loss = SobelLoss().to(self.device)
			self.add_metric(self.train_metric, "sobel_loss")
			self.add_metric(self.val_metric, "sobel_loss")
	...
	def init_pose_loss(self, yolo_path, model_path, model_sz='b', max_batch_size=64):
		return PoseLoss(yolo_path=yolo_path, model_path=model_path, model_sz=model_sz, max_batch_size=max_batch_size).to(self.device)
	...
	def train(self):        
		for epoch in range(self.num_epoch):
			...
			self.model.train()
			for batch_idx, (im1, im2, im3) in enumerate(tqdm(self.train_loader)):
				if self.debug:
			...
			self.model.eval()
			with torch.no_grad():
				for batch_idx, (im1, im2, im3) in enumerate(tqdm(self.val_loader)):
					if self.debug:
						if batch_idx == 4:
							break
			...
			torch.save(self.model.state_dict(), os.path.join(self.model_checkpoints, f'epoch_{epoch}_psnr_{round(self.val_metric["psnr"],4)}.pt'))