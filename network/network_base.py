...
class Network(nn.Module):
	def __init__(self, global_motion=True, ensemble_global_motion=False):
		...
		motion_mlp_hidden_dim = int(self.fused_dim * 0.75) # 576
		...
		# ----- global motion -----
		last_feat_dim = self.hidden_dims[-1] + 96 # 288
		...
		motion_mlp_hidden_dim = 768 # int(concat_dim * 2 * 0.75) # 576
	...
	def estimate_local_motion(self, feat):
		...
		# import numpy as np
		# import cv2
		# # flow = motion.clone().detach().cpu().numpy()[0].transpose(1,2,0)
		
		# # flow1 = flow_vis.flow_to_color(flow[:,:,0:2])
		# # flow2 = flow_vis.flow_to_color(flow[:,:,2:4])
		# # flow3 = flow_vis.flow_to_color(flow[:,:,4:6])
		# # flow4 = flow_vis.flow_to_color(flow[:,:,6:8])
		# # flow1 = Image.fromarray(flow1)
		# # flow2 = Image.fromarray(flow2)
		# # flow3 = Image.fromarray(flow3)
		# # flow4 = Image.fromarray(flow4)
		# # flow1.save("flow1.png")
		# # flow2.save("flow2.png")
		# # flow3.save("flow3.png")
		# # flow4.save("flow4.png")

		# # flow5 = opt_flow_0.clone().detach().cpu().numpy()[0].transpose(1,2,0)
		# # flow6 = opt_flow_1.clone().detach().cpu().numpy()[0].transpose(1,2,0)
		# # flow5 = flow_vis.flow_to_color(flow5)
		# # flow6 = flow_vis.flow_to_color(flow6)
		# # flow5 = Image.fromarray(flow5)
		# # flow6 = Image.fromarray(flow6)
		# # flow5.save("flow5.png")
		# # flow6.save("flow6.png")

		# mask1 = occ_mask1.clone().detach().cpu().numpy()[0].transpose(1,2,0)
		# mask2 = 1-mask1
	...
	def forward_normal(self, im0, im1):
		...
		# res = I_t_residual.clone().detach().cpu().numpy()[0].transpose(1,2,0)
		# res = (res * 255).astype(np.uint8)
		# cv2.imwrite("res.png", res)

		# it0 = I_t_0.clone().detach().cpu().numpy()[0].transpose(1,2,0)
		# it0 = it0[:,:,::-1].copy()
		# it0 = (it0 * 255).astype(np.uint8)
		# cv2.imwrite("it0.png", it0)

		# it1 = I_t_1.clone().detach().cpu().numpy()[0].transpose(1,2,0)
		# it1 = it1[:,:,::-1].copy()
		# it1 = (it1 * 255).astype(np.uint8)
		# cv2.imwrite("it1.png", it1)
	...
	# 	# loss = torch.mean(l1(im0, im1), dim=[1,2,3]) # [B,C,H,W] -> [B]
		
	# 	loss = torch.mean(l1(im0, im1), dim=[1]) # [B,C,H,W] -> [B,H,W]
	# 	loss = F.interpolate(loss[:,None], scale_factor=1/upsample_factor, mode='bilinear', align_corners=True).squeeze(1)
	# 	return loss
	...
	# 	opt_flow0_l1 = upsample_flow(opt_flow0_l1, upsample_factor=2, mode='bilinear')
	# 	opt_flow1_l1 = upsample_flow(opt_flow1_l1, upsample_factor=2, mode='bilinear')
	# 	loss1 = self.global_alignmentness([opt_flow0_l1, opt_flow1_l1], im0, im1)
	...
	# 	opt_flow0_l2 = upsample_flow(opt_flow0_l2, upsample_factor=4, mode='bilinear')
	# 	opt_flow1_l2 = upsample_flow(opt_flow1_l2, upsample_factor=4, mode='bilinear')
	# 	loss2 = self.global_alignmentness([opt_flow0_l2, opt_flow1_l2], im0, im1)

	# 	opt_flow0 = torch.zeros_like(opt_flow0_l0)
	# 	opt_flow1 = torch.zeros_like(opt_flow1_l0)

	# 	B,_,H,W = opt_flow0_l0.size() # [B, _, H/16, W/16]
	# 	for i in range(B):
	# 		for h in range(H):
	# 			for w in range(W):
	# 				min_loss = min(loss0[i,h,w], loss1[i,h,w], loss2[i,h,w])
	# 				if min_loss == loss0[i,h,w]:
	# 					opt_flow0[i,:,h,w] = opt_flow0_l0[i,:,h,w]
	# 					opt_flow1[i,:,h,w] = opt_flow1_l0[i,:,h,w]

	# 				elif min_loss == loss1[i,h,w]:
	# 					opt_flow0[i,:,h,w] = opt_flow0_l1[i,:,h,w]
	# 					opt_flow1[i,:,h,w] = opt_flow1_l1[i,:,h,w]

	# 				elif min_loss == loss2[i,h,w]:
	# 					opt_flow0[i,:,h,w] = opt_flow0_l2[i,:,h,w]
	# 					opt_flow1[i,:,h,w] = opt_flow1_l2[i,:,h,w]
	...
	def inference(self, im0, im1):
		...
		feat_ = torch.cat([im0, im1], dim=0) # speed up using parallelization
		for scale in range(self.pyramid_level):
			feat_ = self.feat_extracts[scale](feat_)
			if scale != 0:
				feat_scale_level.append(feat_)
		...
		if self.global_motion:
			feat_ = self.last_feat_extract(feat_)
			feat_scale_level.append(feat_)
			feat_scale_level.pop(0)
			feat_, h_, w_ = self.global_feature_fusion(feat_scale_level) # [B (HW) C]
			feat_ = einops.rearrange(feat_, 'B (H W) C -> B H W C', H=h_)
			...
			motion = []
			for k, blk in enumerate(self.global_motion_transformer):
				_, h_, w_, _ = feat_.size()
				feat_, x_motion = blk(feat_, h_, w_, B)
				if k == 0:
					feat_ = einops.rearrange(feat_, 'B (H W) C -> B H W C', H=h_)
				x_motion = einops.rearrange(x_motion, '(N B) L K -> B L (N K)', N=2)
				motion.append(x_motion)
		...
		motion = torch.cat(motion, dim=2)
		motion = einops.rearrange(motion, 'B (H W) C -> B C H W', H=h)
		out = self.local_motion_mlp(torch.cat([motion, feat], dim=1))
		opt_flow_0 = out[:, :2]
		opt_flow_1 = out[:, 2:4]
		occ_mask1 = torch.sigmoid(out[:, 4].unsqueeze(1))
		occ_mask2 = 1 - occ_mask1
...
if __name__ == "__main__":
	...
	unet.global_motion = True
	# unet.global_motion = False
